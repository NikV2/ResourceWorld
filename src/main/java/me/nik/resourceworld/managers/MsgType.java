...
public enum MsgType {
    ...
    RESOURCEWORLD_SPAWN_TELEPORT(PREFIX.getMessage() + Messenger.format(ResourceWorld.getInstance().getLang().getString("resourceworld_spawn_teleport"))),
    SPAWN_SET(PREFIX.getMessage() + Messenger.format(ResourceWorld.getInstance().getLang().getString("spawn_set"))),
    INVALID_SPAWN(PREFIX.getMessage() + Messenger.format(ResourceWorld.getInstance().getLang().getString("invalid_spawn"))),
